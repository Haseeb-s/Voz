 ...
 *
 */
public class MoralSupport // class that can be used to print one line of formatted
// output
{
...
// 	} // End function printString
	...
	/**
	 * Purpotse: Checks for a lower bound as well as an upper bound
	 * @param
	 * @return
	 */
	public static int intCheck (int lower, int upper){
		int toCheck = 0;
		boolean valid = false;
		...
		do{
			try{
				toCheck = intCheck();
				if (toCheck > upper || toCheck < lower)
					throw new Exception();
				else
					valid = true;
			}
			catch(Exception e){
				System.err.println("Please enter a number that ranges "+ lower +" - " + upper);
			}
		}while(!valid);
...
//		System.out.println("clean?");
	...
	public static String validateFileName(){
		...
		do{
			fileName = MoralSupport.queryString("Name the file you wish to work with:", false);
			try {
